...
module.exports = {
  ...
  isOfficialUser: (req, res, next) => {
    if (req.isAuthenticated() && (req.user.isOfficial || req.user.isAdmin)) {
      return next();
    }
    req.flash('error', 'Za pristup ovom dijelu aplikacije, morate biti prijavljeni kao službeni korisnik!'); // we can add this before res.redirect or in the same line as res.render like: return res.render("register", {"error": err.message});
    res.redirect('back');
  },

  isOfficialUserConfirmed: (req, res, next) => {
    if (req.isAuthenticated() && ((req.user.isOfficial && req.user.isConfirmed) || req.user.isAdmin)) {
      return next();
    }
    req.flash('error', 'Niste još potvrđeni kao službeni korisnik!'); // we can add this before res.redirect or in the same line as res.render like: return res.render("register", {"error": err.message});
    res.redirect('back');
  },
    ...
    Campground.findOne(
      ...
      (err, foundCampground) => {
        if (err || !foundCampground) {
          // error or foundcampground is null
